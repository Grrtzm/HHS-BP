...
void bepaalPositieMetAccelerometer(int& vogel) {
  ...
  // "Waterpas" demo;
  // Kies welke accelerometer[] waarde je gaat gebruiken (gebruik daarvoor MicrobitV2_LSM303AGR_DataLog_Plotter.ino).
  // We vertalen deze accelerometer waarde met de map() functie naar een waarde van 0 t/m 4 (led-coördinaten).
  // Als je uitleg wilt van de Arduino map() functie, selecteer dan het woord 'map' en druk Ctrl+Shift+F

  vogel = map(0, 0, 0, 0, 0);  // hier gebeurt alle magie. Wijzig deze regel en kijk wat verder nodig is.
}